...
def prepare_dataset(family_name, chrom_name, genome, sample_size=5000):
...
selected_chrom = '1'
recall_result = []
fig1, ax1 = plt.subplots(nrows=3, ncols=2, figsize=(14, 8))
...
for family, fam_idx in zip(selected_families, range(len(selected_families))):
    ...
    sel_enc_seqs = enc_seqs[idx_pred_correct_by_both][:1000]
    print(sel_enc_seqs.shape)
    recall_result.append([family, 'binary GN', len(np.where(preds_binary_gn == 1)[0])/len(preds_binary_gn)])
    recall_result.append([family, 'binary DS', len(np.where(preds_binary_dsn == 1)[0])/len(preds_binary_dsn)])
    recall_result.append([family, 'multi', len(np.where(preds_multi == 1)[0])/len(preds_multi)])
...
fig1.tight_layout()
plt.savefig(f"results/Figures/binary_vs_multilabel_map.svg", bbox_inches='tight', dpi=300, format='svg')
plt.show()

recall_result = pd.DataFrame(recall_result, columns=['Family', 'model', 'recall'])
print(recall_result.head())
fig2, ax2 = plt.subplots(nrows=1, ncols=1, figsize=(8, 4))
sns.barplot(data=recall_result, x='Family', y='recall', hue='model', ax=ax2, palette=palettes,
            hue_order=['multi', 'binary GN', 'binary DS'])
ax2.yaxis.grid(True, alpha=0.3)
ax2.xaxis.grid(True, alpha=0.3)
ax2.set_axisbelow(True)
ax2.set_ylim(0.5, 1.0)
fig2.tight_layout()
plt.savefig(f"results/Figures/binary_vs_multilabel_perf.svg", bbox_inches='tight', dpi=300, format='svg')
plt.show()
