...
sns.set_context(context="paper", rc={"font.size":8,"axes.titlesize":8,"axes.labelsize":8})
...
data['col'] = [int(x.split('.')[0]) for x in data.index.tolist()]
...
for idx in range(5):
    sns.heatmap(data=data[data['col'] == idx+1][tf_families], ax=ax[idx], cmap='coolwarm', yticklabels=False)
fig.tight_layout()
plt.savefig(f"results/Figures/snp_effects_on_cis_regions.png", bbox_inches='tight', dpi=300, format='png')
plt.show()
...
ax2.set_xlim(0, 100)
fig2.tight_layout()
plt.savefig(f"results/Figures/snp_effects_on_cis_regions_count_plot.svg", bbox_inches='tight', dpi=300, format='svg')
plt.savefig(f"results/Figures/snp_effects_on_cis_regions_count_plot.png", bbox_inches='tight', dpi=300, format='png')
